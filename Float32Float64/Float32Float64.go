...
func New(ar []float64) []KeyVal {
	newar := make([]KeyVal, len(ar))
	for i, v := range ar {
		newar[i] = KeyVal{float32(i), v}
	}
	return newar
}

func Fill(ar []float64, newar []KeyVal) []KeyVal {
	if cap(newar) < len(ar) {
		newar = make([]KeyVal, len(ar))
	} else {
...
func Keys(ar []float32, newar []KeyVal) []float32 {
	l := len(newar)
	if len(ar) < l {
		ar = make([]float32, l)
	}
