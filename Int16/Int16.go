...
func heapSortAsc(data []int16, a, b int) {
	first := a
	lo := 0
	hi := b - a
	for i := (hi - 1) / 2; i >= 0; i-- {
...
func insertionSortAsc(data []int16, a, b int) {
	var j int
	for i := a + 1; i < b; i++ {
...
func siftDownAsc(data []int16, lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func medianOfThreeAsc(data []int16, a, b, c int) {
	m0 := b
	m1 := a
	m2 := c
...
func swapRangeAsc(data []int16, a, b, n int) {
	for i := 0; i < n; i++ {
...
func doPivotAsc(data []int16, lo, hi int) (midlo, midhi int) {
	m := lo + (hi-lo)/2
	if hi-lo > 40 {
		s := (hi - lo) / 8
...
func quickSortAsc(data []int16, a, b, maxDepth int) {
	var mlo, mhi int
	for b-a > 7 {
		if maxDepth == 0 {
...
func IsSortedAsc(data []int16) bool {
...
func symMergeAsc(data []int16, a, m, b int) {
	if a >= m || m >= b {
		return
	}
	mid := a + (b-a)/2
...
func rotateAsc(data []int16, a, m, b int) {
	i := m - a
	if i == 0 {
		return
	}
...
func heapSortDesc(data []int16, a, b int) {
	first := a
	lo := 0
	hi := b - a
	for i := (hi - 1) / 2; i >= 0; i-- {
...
func insertionSortDesc(data []int16, a, b int) {
	var j int
	for i := a + 1; i < b; i++ {
		for j = i; j > a && data[j] > data[j-1]; j-- {
			data[j], data[j-1] = data[j-1], data[j]
...
func siftDownDesc(data []int16, lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func medianOfThreeDesc(data []int16, a, b, c int) {
	m0 := b
	m1 := a
	m2 := c
	// bubble sort on 3 elements
...
func swapRangeDesc(data []int16, a, b, n int) {
	for i := 0; i < n; i++ {
		data[a], data[b] = data[b], data[a]
		a++
		b++
...
func doPivotDesc(data []int16, lo, hi int) (midlo, midhi int) {
	m := lo + (hi-lo)/2
	if hi-lo > 40 {
		s := (hi - lo) / 8
...
func quickSortDesc(data []int16, a, b, maxDepth int) {
	var mlo, mhi int
	for b-a > 7 {
		if maxDepth == 0 {
...
func IsSortedDesc(data []int16) bool {
	for i := len(data) - 1; i > 0; i-- {
		if data[i] > data[i-1] {
			return false
		}
...
func symMergeDesc(data []int16, a, m, b int) {
	if a >= m || m >= b {
		return
	}
	mid := a + (b-a)/2
...
func rotateDesc(data []int16, a, m, b int) {
	i := m - a
	if i == 0 {
		return
	}
